...
// Générateur sûr d'obstacles : construit et renvoie des pointeurs `Man*`
// sans appeler d'API spécifique (positionnement / enregistrement au simulateur).
#ifdef GL
static std::vector<Man*> generateObstaclesSafe(
    int count,
    ...
    float z = 0.0f)
{
  ...
  for (int i = 0; i < count; ++i) {
    ...
    Man* o = new Man(std::string("obs_") + std::to_string(i), /*id*/ i + 1);
